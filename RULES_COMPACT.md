...
- `IPG` in headline — acceptable abbreviation for `Initial Price Guidance` / `Initial Guidance`; NEVER flag as wrong or require the full form (Bloomberg blasts use IPG); body opener `Initial guidance is X% area for...` is equally valid — NEVER rewrite to `IPTs are...`
